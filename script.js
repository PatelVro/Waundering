...
// Full-dashboard refresh every 2 minutes — picks up new Elo, recent matches,
// predictions regenerated by the server-side export loop.
function startFullRefresh() {
  if (_fullRefreshTimer) return;
  _fullRefreshTimer = setInterval(async () => {
    try {
      const data = await loadData();
      // Only re-hydrate everything if the export script ran again
      if (data.generated_at !== _lastGeneratedAt) {
        hydrateAll(data);
        _lastGeneratedAt = data.generated_at;
        // Re-wire live timer if match state changed
        if (!_liveRefreshTimer && data.live_match && !data.live_match.is_complete) {
          startLiveRefreshIfNeeded(data.live_match);
        }
      } else {
        // Even if export didn't re-run, refresh the live section (tracker updates it independently)
        hydrateLive(data.live_match);
      }
    } catch (_) {}
  }, 120_000); // 2 minutes
...
function hydrateLive(live) {
  ...
  // Toss
  const tossEl = $("#live-toss");
  if (live.toss_winner) {
    const dec = live.toss_decision === "field" ? "bowl" : (live.toss_decision || "bat");
    tossEl.textContent = `Toss: ${escape(live.toss_winner)} elected to ${dec}`;
    tossEl.style.display = "";
  } else {
    tossEl.style.display = "none";
  }

  // Playing XIs
  const xiWrap = $("#live-xi-wrap");
  const homeXi = live.home_xi || [];
  const awayXi = live.away_xi || [];
  if (homeXi.length || awayXi.length) {
    $("#live-xi-home-label").textContent = escape(live.home || "Home");
    $("#live-xi-away-label").textContent = escape(live.away || "Away");
    const renderXi = (listEl, xi) => {
      listEl.innerHTML = xi.map(n => `<li>${escape(n)}</li>`).join("");
    };
    renderXi($("#live-xi-home"), homeXi);
    renderXi($("#live-xi-away"), awayXi);
    xiWrap.style.display = "";
  } else {
    xiWrap.style.display = "none";
  }
