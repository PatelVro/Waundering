...
def train(format_filter: str | None = "IT20", limit: int | None = None) -> dict:
    ...
    print(f"  rows: {len(df):,}")
    train_df, test_df = F.split_by_date(df, test_frac=0.2)
    print(f"  train: {len(train_df):,}   test: {len(test_df):,}")

    X_tr, X_te = _matrix(train_df), _matrix(test_df)
    ...
    runs_pred = runs.predict(X_te)
    wkt_pred  = wkt.predict(X_te)

    metrics = {
        "rows_train":   int(len(train_df)),
        "rows_test":    int(len(test_df)),
        "runs_logloss": float(log_loss(test_df["y_runs_bucket"], runs_pred,
                                       labels=list(range(len(RUN_BUCKETS))))),
        "runs_top1":    float(accuracy_score(test_df["y_runs_bucket"],
                                             np.argmax(runs_pred, axis=1))),
        "wicket_logloss": float(log_loss(test_df["y_wicket"], wkt_pred)),
